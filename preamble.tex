...
% User variables
\newcommand{\MaxInterludes}{20} % Define the maximum number of interludes
\newcommand{\PDFList}{} % User-defined list of PDFs, make sure there are no spaces between the commas and each filename ends in .pdf
...
\clist_map_inline:Nn \PDFList
 {
  \seq_gput_right:Nn \g_pdf_files_seq { \tl_to_str:n {#1} }
 }
...
% Lists all of the PDFs and their index number
\NewDocumentCommand{\listpdfs}{}
 {
  \seq_indexed_map_inline:Nn \g_pdf_files_seq
   {
    \noindent
    Index:~##1,~~File Name:~\textit{##2} \par
   }
 }
...
% End of PDF Automation Template
